...
function _wp_handle_upload(&$file, $overrides, $time, $action) {
	...
	*/
	...
	if (false === $move_new_file) {
		if (0 === stripos($uploads['basedir'], ABSPATH)) {
...
function _unzip_file_ziparchive($file, $to, $needed_dirs = array()) {
	...
	if (!$wp_filesystem) {
		...
		if (empty($wp_filesystem)) {
			send_response_local_sync('FS_INIT_FAILED-021');
			return false;
		}
	}
	$z = new ZipArchive();
	...
	*/
	...
	foreach ($needed_dirs as $dir) {
		...
		if (stripos($dir, $to) === false) // If the directory is not within the working directory, Skip it
		{
			continue;
		}
...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	if (!$wp_filesystem) {
		initiate_filesystem_local_sync();
		if (empty($wp_filesystem)) {
			send_response_local_sync('FS_INIT_FAILED-022');
			return false;
		}
	}
	mbstring_binary_safe_encoding();

	require_once ABSPATH . BRIDGE_NAME_LOCAL_SYNC . '/wp-files/class-pclzip.php'; //modified
...
function copy_dir($from, $to, $skip_list = array()) {
	...
	if (!$wp_filesystem) {
		initiate_filesystem_local_sync();
		if (empty($wp_filesystem)) {
			send_response_local_sync('FS_INIT_FAILED-023');
			return false;
		}
	}
	...
	foreach ((array) $dirlist as $filename => $fileinfo) {
		...
		if ('f' == $fileinfo['type']) {
			...
			foreach ($skip_list as $skip_item) {
				if (0 === stripos($skip_item, $filename . '/')) {
...
function WP_Filesystem($args = false, $context = false, $allow_relaxed_file_ownership = false) {
	...
	//require_once ABSPATH . BRIDGE_NAME_LOCAL_SYNC . '/wp-files/class-wp-filesystem-base.php'; //modified
	...
	if (!class_exists("WP_Filesystem_$method")) {
		 ...
		 */
		$abstraction_file = apply_filters('filesystem_method_file', ABSPATH . BRIDGE_NAME_LOCAL_SYNC . '/wp-files/class-wp-filesystem-' . $method . '.php', $method); //modified
