...
if (Duplicator\Requirements::canRun($currentPluginBootFile) === false) {
    ...
    require_once(__DIR__ . '/lib/snaplib/snaplib.all.php');
    ...
    require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/utilities/class.u.migration.php');
    ...
    add_action('plugins_loaded', array(\Duplicator\Core\Addons\Manager::getInstance(), 'inizializeAddons'));
    ...
    if (!empty($dpro_license_key)) {
        ...
        if (($global !== null) &&
            ...
            $edd_updater = new EDD_SL_Plugin_Updater(EDD_DUPPRO_STORE_URL, DUPLICATOR____FILE, $dpro_edd_opts, DUP_PRO_Constants::PLUGIN_SLUG);
    ...
    if (is_admin()) {
        ...
        register_deactivation_hook(DUPLICATOR____FILE, 'duplicator_pro_deactivate');
...
HTML;
        ...
        function duplicator_pro_menu()
        {
            ...
            //Main Menu
            $export_perms_txt = 'export';
            $export_perms     = apply_filters($wpfront_caps_translator, $export_perms_txt);

            $main_menu = add_menu_page('Duplicator Plugin', 'Duplicator Pro', $export_perms, DUP_PRO_Constants::PLUGIN_SLUG, 'duplicator_pro_get_menu', DUP_PRO_Constants::ICON_SVG);

            $lang                              = DUP_PRO_U::__('Packages');
            $page_packages                     = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, $lang, $lang, $export_perms, DUP_PRO_Constants::$PACKAGES_SUBMENU_SLUG, 'duplicator_pro_get_menu');
            $GLOBALS['DUP_PRO_Package_Screen'] = new DUP_PRO_Package_Screen($page_packages);

            if (!DUP_PRO_CTRL_import_installer::isDisallow()) {
                $import_perms_txt = 'import';
                $import_perms     = apply_filters($wpfront_caps_translator, $import_perms_txt);
            ...
            $page_schedules = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, $lang, $lang, $export_perms, DUP_PRO_Constants::$SCHEDULES_SUBMENU_SLUG, 'duplicator_pro_get_menu');

            $lang         = DUP_PRO_U::__('Storage');
            $page_storage = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, $lang, $lang, $export_perms, DUP_PRO_Constants::$STORAGE_SUBMENU_SLUG, 'duplicator_pro_get_menu');

            $lang       = DUP_PRO_U::__('Tools');
            $page_tools = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, $lang, $lang, $export_perms, DUP_PRO_Constants::$TOOLS_SUBMENU_SLUG, 'duplicator_pro_get_menu');

            $lang          = DUP_PRO_U::__('Settings');
            $page_settings = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, $lang, $lang, $export_perms, DUP_PRO_Constants::$SETTINGS_SUBMENU_SLUG, 'duplicator_pro_get_menu');

            if ($global->debug_on) {
                $lang       = DUP_PRO_U::__('Debug');
                $page_debug = add_submenu_page(DUP_PRO_Constants::PLUGIN_SLUG, $lang, $lang, $export_perms, DUP_PRO_Constants::$DEBUG_SUBMENU_SLUG, 'duplicator_pro_get_menu');
