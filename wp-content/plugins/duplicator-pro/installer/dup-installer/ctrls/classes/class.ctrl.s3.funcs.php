...
final class DUPX_S3_Funcs
{
     ...
     * @var int|nul; 
     */
    private $actionMuMode = null;

    private function __construct()
    {
        $this->timeStart    = DUPX_U::getMicrotime();
        $this->actionMuMode = DUPX_MU::newSiteAction();
    }
    ...
    private function setMultisiteReplaceList()
    {
        ...
        DUPX_Log::info("-----------------------------------------", 2);
        DUPX_Log::info("ACTION MU MODE START :\"{$this->actionMuMode}\"", 2);

        switch ($this->actionMuMode) {
            case DUPX_MultisiteMode::Subdomain:
            case DUPX_MultisiteMode::Subdirectory:
                if ($this->actionMuMode === DUPX_MultisiteMode::Subdomain) {
            ...
            case DUPX_MultisiteMode::Standalone:
                ...
                if ($subsite_id == 1) {
                    ...
                    if (!$paramsManager->getValue(DUPX_Paramas_Manager::PARAM_SKIP_PATH_REPLACE)) {
                ...
                try {
                    DUPX_MU::convertSubsiteToStandalone($subsite_id);
                }
                catch (Exception $ex) {
    ...
    {
        $mainOld = DupProSnapLibIOU::safePath($mainOld);
        $mainNew = DupProSnapLibIOU::safePath($mainNew);
        $old     = DupProSnapLibIOU::safePath($old);
        $new     = DupProSnapLibIOU::safePath($new);
    ...
    {
        DUPX_Log::info('RESET ALL USERS SESSION TOKENS');
        $escapedTablePrefix = mysqli_real_escape_string($this->dbh, DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_DB_TABLE_PREFIX));

        try {
            DUPX_DB::mysqli_query($this->dbh, "DELETE FROM `".$escapedTablePrefix."usermeta` WHERE meta_key='session_tokens'");
        }
        catch (Exception $e) {
            DUPX_Log::info('RESET USER SESSION TOKENS EXCEPTION: '.$e->getMessage());
        }
    }
    ...
    public function configFilesUpdate()
    {
        ...
        if (DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_WP_CONFIG) == 'nothing') {
            ...
            if (DupProSnapLibIOU::chmod($wpConfigFile, 'u+rw') === false) {
    ...
    protected function indexPhpUpdate()
    {
        ...
        if (!is_writable($indexPath)) {
            DUPX_Log::info('index.php isn\'t writable');
            return false;
        }

        if (($relativeAbsPath = DupProSnapLibIOU::getRelativePath($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_PATH_WP_CORE_NEW), $pathNew)) === false) {
            $blogHeaderValue = "'".$paramsManager->getValue(DUPX_Paramas_Manager::PARAM_PATH_WP_CORE_NEW)."/wp-blog-header.php'";
        } else {
        ...
        if (($indexContent = file_get_contents($indexPath)) === false) {
            DUPX_Log::info('Can\'t read index.php content');
            return false;
        }
        ...
        if (file_put_contents($indexPath, $indexContent) === false) {
            DUPX_Log::info('Can\'t update index.php content');
            return false;
        }

        DUPX_Log::info('INDEX.PHP updated with new blog header '.DUPX_Log::varToString($blogHeaderValue)."\n");
        return true;
    }
    ...
    protected function wpConfigUpdate(DupProWPConfigTransformer $confTransformer)
    {
        ...
        try {
            ...
            DUPX_Log::info('UPDATE DB_CHARSET '.DUPX_Log::varToString($dbcharset));

            $confTransformer->update('constant', 'DB_COLLATE', $dbcollate);
            DUPX_Log::info('UPDATE DB_COLLATE '.DUPX_Log::varToString($dbcollate));
            ...
            $updateAuthKeys = ($archiveConfig->getLicenseType() >= DUPX_LicenseType::Freelancer && $this->post['auth_keys_and_salts']);
            if ($updateAuthKeys) {
            ...
            DUPX_ArchiveConfig::updateWpConfigByParam($confTransformer, 'WP_TEMP_DIR', DUPX_Paramas_Manager::PARAM_WP_CONF_WP_TEMP_DIR);
...
LONGMSG;
    ...
    protected function configurationMultisiteUpdate(DupProWPConfigTransformer $confTransformer)
    {
        ...
        } else if (DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_WP_CONFIG) == 'new') {
            DUPX_Log::info('TRANSFORMER[wpmu]: new wp-config from sample');
            $archiveConfig = DUPX_ArchiveConfig::getInstance();
    ...
    protected function configurationUrlsAndPaths(DupProWPConfigTransformer $confTransformer)
    {
        ...
        if ($confTransformer->exists('constant', 'ABSPATH')) {
            if (($relativeAbsPath = DupProSnapLibIOU::getRelativePath($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_PATH_WP_CORE_NEW), $pathNew)) === false) {
                $absPathValue = "'".$paramsManager->getValue(DUPX_Paramas_Manager::PARAM_PATH_WP_CORE_NEW)."'";
            } else {
                $absPathValue = "dirname(__FILE__) . '/".$relativeAbsPath."'";
            }
            $confTransformer->update('constant', 'ABSPATH', $absPathValue, array('raw' => true));
            DUPX_Log::info('UPDATE ABSPATH '.DUPX_Log::varToString($absPathValue));
        }
    ...
    protected function htaccessUpdate()
    {
        ...
        DUPX_ServerConfig::setup($this->actionMuMode, $GLOBALS['DUPX_AC']->mu_generation, $this->dbh, $this->post['path_new']);
    }
    ...
    {
        DUPX_Log::info('MIGRATION INFO SET');
        // make sure dbConnection is initialized
        $this->dbConnection();

        $paramsManager = DUPX_Paramas_Manager::getInstance();

        $optionTable   = mysqli_real_escape_string($this->dbh, $paramsManager->getValue(DUPX_Paramas_Manager::PARAM_DB_TABLE_PREFIX)).'options';
        ...
        $query = "REPLACE INTO `".$optionTable."` (`option_id`, `option_name`, `option_value`, `autoload`) VALUES ".
            ...
            "(NULL, '".self::MIGRATION_DATA_OPTION."', '".mysqli_real_escape_string($this->dbh, DupProSnapJsonU::wp_json_encode_pprint($migrationData))."', 'no');";
    ...
    public function generalCleanup()
    {
        ...
        if ($this->actionMuMode == DUPX_MultisiteMode::Standalone) {
    ...
    private function wpConfigFinalReport()
    {
        ...
        } else if (($new = file_get_contents($wpConfigFile)) === false) {
            $new = 'Can read wp-config.php file';
        } else {
