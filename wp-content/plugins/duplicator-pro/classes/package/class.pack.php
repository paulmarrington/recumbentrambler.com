...
class DUP_PRO_Package
{
    ...
    public function create_scan_report()
    {
        ...
        try {
            ...
            $report['ARC']['Status']['HasImportableSites']     = DupProSnapLibUtil::in_array_extended($report['ARC']['Subsites'], function ($subsite) {
                return count($subsite->filtered_tables) === 0 && count($subsite->filtered_paths) === 0;
            });
            $report['ARC']['Status']['HasNotImportableSites']  = DupProSnapLibUtil::in_array_extended($report['ARC']['Subsites'], function ($subsite) {
                return count($subsite->filtered_tables) > 0 || count($subsite->filtered_paths) > 0;
            });
            ...
            $report['ARC']['Status']['PackageIsNotImportable'] = !($report['ARC']['Status']['CanbeMigratePackage'] && !$this->Archive->ExportOnlyDB
                && $report['SRV']['WP']['core'] && (!$report['ARC']['Status']['HasFilteredSiteTables'] || $report['ARC']['Status']['HasImportableSites'])
                && (!$report['ARC']['Status']['HasNotImportableSites'] || DUP_PRO_License_U::getLicenseType() === DUP_PRO_License_Type::BusinessGold));
    ...
    public function build_integrity_check()
    {
        ...
        if (!strstr($sql_done_txt, DUPLICATOR_PRO_DB_EOF_MARKER) ||
            ...
            if ($this->Database->DBMode == 'MYSQLDUMP') {
                ...
                $system_global->add_recommended_quick_fix($error_text, $fix_text,
                    array(
                        'global' => array(
                            'package_mysqldump'          => 0,
                ...
                if (($straight_ratio < 0.90) || ($straight_ratio > 1.01)) {
                    ...
                    if (($warning_ratio < 0.90) || ($warning_ratio > 1.01)) {
                        ...
                        if ($this->build_progress->current_build_mode == DUP_PRO_Archive_Build_Mode::Shell_Exec) {
                            ...
                            $system_global->add_recommended_quick_fix($error_message, $fix_text,
                                array(
                                    'global' => array(
            ...
            if ($files !== false) {
                foreach ($files as $file_path) {
                    ...
                    if (filemtime($file_path) <= time() - DUP_PRO_Constants::TEMP_CLEANUP_SECONDS) {
                        DupProSnapLibIOU::rrmdir($file_path);
                    }
                }
            }
        }
