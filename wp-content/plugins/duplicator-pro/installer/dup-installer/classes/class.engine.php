...
MSG;
    ...
    public static function replaceSiteTable()
    {
        ...
        $prefix    = $paramsManager->getValue(DUPX_Paramas_Manager::PARAM_DB_TABLE_PREFIX);
        $siteTalbe = $prefix . 'site';
        if (!in_array($siteTalbe, DUPX_DB::getTables($dbh))) {
            DUPX_Log::info("NOT SITE TABLE[" . $siteTalbe . "] FOUND, SKIP UPDATE");
            return;
        }
        DUPX_Log::info("ENGINE UPDATE SITE TABLE");

        $escapedSiteTable = mysqli_real_escape_string($dbh, $siteTalbe);

        $rUrlInfo = parse_url($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_URL_NEW));
        ...
        if (DUPX_DB::mysqli_query($dbh, $sql) === false) {
            ...
            DUPX_Log::info('ENGINE SITE TALBE UPDATED ID:' . $networkId . ' DOMAIN:' . $rHost . ' PATH:' . $rPath, DUPX_Log::LV_DETAILED);
        }
    }
